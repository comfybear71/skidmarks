 ...
 *
 * When STUDIO_USERS is set, every other path needs a signed-in cookie.
 * APIs get 401. Pages bounce to /login. Unauthenticated requests must not
 * fall through to Stuie's packs.
 */
export async function proxy(request: NextRequest) {
  ...
  requestHeaders.set(STUDIO_OWNER_HEADER, sess.id);
  return NextResponse.next({
