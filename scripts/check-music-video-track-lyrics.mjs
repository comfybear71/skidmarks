...
// ── Sections come out of the sheet ─────────────────────────────────────────
{
  const { lyricTagsFrom, sectionsFromLyricTags } = await import(
    "../src/lib/musicVideoTrack.ts"
  );
  const newId = (i) => `m${i}`;
  const song = 267_500; // 4:27.5

  const sheet = [
    "[Instrumental Intro] [Tension-strummed 12-string, slow dragging bass pulse]",
    "[Verse 1]Silver glass",
    "[Chorus]Blow the glass clean",
    "[Verse 2]More words",
    "[Chorus]Again",
    "[Sax break]",
    "[Outro]",
    "[Fading drone, dying out]",
  ].join("\n");

  const made = sectionsFromLyricTags({ tags: lyricTagsFrom(sheet), songMs: song, newId });

  // The running order, and stage directions left out of it.
  assert.deepEqual(
    made.map((m) => m.label),
    ["intro", "verse", "chorus", "verse", "chorus", "sax_break", "outro"],
  );

  // End to end, covering the whole song, no gaps and no overlaps.
  assert.equal(made[0].startMs, 0);
  assert.equal(made[made.length - 1].endMs, song, "the last section reaches the end");
  for (let i = 1; i < made.length; i++) {
    assert.equal(made[i].startMs, made[i - 1].endMs, `section ${i} starts where ${i - 1} ends`);
    assert.ok(made[i].endMs > made[i].startMs, "no backwards section");
  }
  assert.equal(new Set(made.map((m) => m.id)).size, made.length, "ids are unique");

  // Nothing to build from is nothing built — never a stray empty marker.
  assert.deepEqual(sectionsFromLyricTags({ tags: [], songMs: song, newId }), []);
  assert.deepEqual(
    sectionsFromLyricTags({ tags: lyricTagsFrom("[Fading drone]"), songMs: song, newId }),
    [],
    "stage directions alone make no sections",
  );
  // No song, no timings to hand out.
  assert.deepEqual(
    sectionsFromLyricTags({ tags: lyricTagsFrom(sheet), songMs: 0, newId }),
    [],
  );

  // The button is offered only while the list is empty, so typed times are
  // never overwritten.
  const { readFileSync } = await import("node:fs");
  const ui = readFileSync(
    new URL("../src/components/mobile/MusicVideoTrack.tsx", import.meta.url),
  ...
  assert.match(ui, /!markers\.length && lyricTags\.length/);
}

console.log("check-music-video-sections-from-lyrics OK");
