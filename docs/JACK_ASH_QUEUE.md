# Jack Ash — song queue

Logged **2026-08-24** from Stuie. Knowledge only. Do **not** mint a new job. Do **not** Start directing. Do **not** cook LTX until he names the test.

Code lock: `src/lib/jackAshQueue.ts`.

## Order

| # | Song | Status 2026-08-24 |
|---|---|---|
| 1 | **MY NEW TOY** | Listed. No job yet. Do not start. |
| 2 | **FORGOTTEN** | Live job `mgen_20260824085817084_edp`. Pack `THE JACK ASH BAND — FORGOTTEN 84_edp`. Mp3 on the job (4:51). Lyrics already there. |
| 3 | **BURNING BRIGHT** | Listed. No job yet. Do not start. |
| 4 | **EAST** | Listed. No job yet. Do not start. |
| 5 | **GIVE ME SOMETHING** | Listed. No job yet. Do not start. |

Same band for all five unless he says otherwise: **THE JACK ASH BAND** — JACK GHOST, SAXOPHONE, DRUMMER, GUITAR, HORN. `styleId: music_video`.

Already live, do not wipe: **BLOWING UP CLAUDE** (`mgen_20260822085033162_0ud`). Not on this queue.

## Base for the next song

Forgotten is the test bed. The next 5–6 songs use `docs/MUSIC_VIDEO_BASE.md`: Crash Lab cameras (OTS, tight CU, MCU, sit, wide, two-shot, three-shot), named instruments, empty-handed Jack, anti-cel. Do not fall back to the generic “half turned in profile” Start line.

**2026-08-24 later:** he marked Forgotten by ear. No sax on this mix. Jack only on vocal hits. Horn fade in / play / fade out / revolve. Intermissions get the concert-loop animation. Clocks: `src/lib/forgottenWhoPlays.ts`. Do not generate T4/T5 yet.

## Study first

He said study requirements before we make Jack Ash videos. The new visual floor is **concert loop plates** (Double Talkin' Jive / Bangkok Thunderdome gold-skull LED loops, no band on them) **plus** the people stills. See `docs/CONCERT_LOOP_PLATE.md`.

**2026-08-24:** he pasted the full I2V / LTX-2.5 notes and said study now, implement after a few more video test runs. PRs #310 and #311 are done. Do not treat a Jack Ash video as “five member stills + Generate cuts” only.
