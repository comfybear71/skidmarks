...
| **T6** | Who-plays + intermission | Jack on vocal hits only (LTX IA2V + mix). Trumpet clocks stay, but play is pose-drive from a real player clip — not LTX text. Face off-camera or shadow. Silent clip, sync in Resolve. Gaps = Grok / dance. No sax. | Clocks in `src/lib/forgottenWhoPlays.ts`. Waiting on his trumpet reference clip. |
